...
export function parseOpenAiApiStdoutLine(line: string, ts: string): TranscriptEntry[] {
  ...
  if (type === "system") {
    ...
    if (subtype === "init") {
      return [{ kind: "init", ts, model: asString(parsed.model, "openai"), sessionId: "" }];
    }
    if (subtype === "error") {
  ...
  if (type === "result") {
    ...
    const inputTokens = asNumber(usageRaw?.inputTokens ?? usageRaw?.prompt_tokens);
    const outputTokens = asNumber(usageRaw?.outputTokens ?? usageRaw?.completion_tokens);
